...
def sinusoidal_embedding(dim, frame_limit):
    ...
    pe = torch.FloatTensor(
        [
            [1 * 2 ** (i // 2) * math.pi * p for i in range(dim)]
            for p in range(frame_limit)
        ]
    )
        ...
        for l in range(num_layers):
            if l != num_layers - 1:
                nn.init.kaiming_normal_(
                    self.net[l].weight, mode="fan_in", nonlinearity="relu"
                )
            else:
...
class PoseField(nn.Module):
    def __init__(
        self, input_dim, hidden_dims, output_dim, frames, pose_mlp_args, *args, **kwargs
    ) -> None:
        ...
        self.twice_std_dev = (
            torch.load(f"4d/poses/{pose_mlp_args.action}_std_dev.pt").cuda().view(330)
        )
...
class DLMesh(nn.Module):
    def __init__(self, opt, num_layers_bg=2, hidden_dim_bg=16):
        ...
        if self.opt.lock_geo:  # texture
            ...
            if self.opt.initialize_pose == "diving":
                self.diving_body_pose = pickle.load(open("4d/poses/diving.pkl", "rb"))[
                    "body_pose"
                ]
            elif self.opt.initialize_pose == "running":
                self.diving_body_pose = pickle.load(open("4d/poses/running.pkl", "rb"))[
                    "body_pose"
                ]
                ...
                self.diving_body_pose += np.random.normal(
                    0, 0.1, self.diving_body_pose.shape
                )
                ...
                self.opt.initialize_pose == "running_mean"
                or self.opt.initialize_pose == "punching_mean"
            ):
                self.diving_body_pose = pickle.load(
                    open(f"4d/poses/{self.opt.initialize_pose}.pkl", "rb")
                )
                self.diving_body_pose = np.repeat(
                    self.diving_body_pose, self.num_frames, axis=0
                )
                ...
                self.diving_body_pose = pickle.load(
                    open(f"4d/poses/{self.opt.action}_mean.pkl", "rb")
                )
                self.diving_body_pose = np.repeat(
                    self.diving_body_pose, self.num_frames, axis=0
                )

            self.diving_body_pose = (
                torch.as_tensor(self.diving_body_pose).float().to(self.device)
            )
                    ...
                    if self.opt.use_full_pose:
                        self.init_full_pose_6d = torch.cat(
                            [
                                ...
                                matrix_to_rotation_6d(
                                    axis_angle_to_matrix(
                                        torch.as_tensor(smplx_params["body_pose"]).view(
                                            -1, 3
                                        )
                                    ).to(self.device)
                                ),  # body
                                matrix_to_rotation_6d(
                                    axis_angle_to_matrix(
                                        torch.as_tensor(smplx_params["jaw_pose"]).view(
                                            -1, 3
                                        )
                                    ).to(self.device)
                                ),  # jaw
                        ...
                        self.full_pose_6d = torch.zeros(
                            self.init_full_pose_6d.shape
                        ).to(self.device)
                    else:
                        if self.vpose:
                            if not self.opt.video:
                                ...
                                self.init_body_pose_6d_set = self.body_prior.encode(
                                    self.diving_body_pose
                                ).mean  # latent space
                                # self.init_body_pose_6d_set = torch.randn(self.diving_body_pose.shape[0],32).to(self.device)
                                self.body_pose_6d_set = torch.zeros(
                                    self.init_body_pose_6d_set.shape
                                ).to(self.device)

                        else:
                            self.init_body_pose_6d = matrix_to_rotation_6d(
                                axis_angle_to_matrix(self.body_pose.view(-1, 21, 3))
                            ).view(1, -1)
                            self.init_body_pose_6d_set = self.init_body_pose_6d.repeat(
                                [self.num_frames, 1]
                            )
                            self.init_body_pose_6d_set = (
                                matrix_to_rotation_6d(
                                    axis_angle_to_matrix(
                                        self.diving_body_pose.view(-1, 3)
                                    )
                                )
                                .view(self.num_frames, -1)
                                .float()
                            )
                            self.prev_pose = self.init_body_pose_6d_set
                            self.prev_pose = torch.cat(
                                [self.prev_pose, self.prev_pose[None, 0]], dim=0
                            )
                            self.body_pose_6d = torch.zeros(
                                self.init_body_pose_6d.shape
                            ).to(self.device)
                            self.body_pose_6d_set = torch.zeros(
                                self.init_body_pose_6d_set.shape
                            ).to(self.device)
                    ...
                    if self.opt.use_full_pose:
                        ...
                        if self.vpose:
                            ...
                            if self.opt.initialize_pose == "zero":
                                if not self.opt.video:
                                    self.body_pose_6d = torch.zeros([1, 32]).to(
                                        self.device
                                    )
                                else:
                                    self.body_pose_6d_set = torch.zeros(
                                ...
                                self.body_pose_6d_set = self.body_prior.encode(
                                    self.diving_body_pose
                                ).mean  # latent space

                        else:
                            self.body_pose_6d = matrix_to_rotation_6d(
                                axis_angle_to_matrix(self.body_pose.view(-1, 21, 3))
                            ).view(1, -1)
                self.body_pose = None
                ...
                if self.vpose:
                    if self.opt.pose_mlp == "pose":
                        self.pose_mlp = PoseField(
                            8, [32, 32], 32, self.num_frames, self.opt.pose_mlp_args
                        )
                    elif self.opt.pose_mlp == "angle":
                        raise NotImplementedError(
                            "AngleField is not implemented yet for vpose"
                        )
                else:
                    if self.opt.pose_mlp == "pose":
                        if self.vpose:
        ...
        if not self.opt.lock_tex:  # texture parameters
            if self.opt.tex_mlp:
                ...
                if False:
                    ...
                    albedo = (
                        torch.ones((res, res, 3), dtype=torch.float32) * 0.5
                    )  # default color
                    ...
                    self.raw_albedo = torch.as_tensor(
                        albedo_image, dtype=torch.float32, device=self.device
                    )
        ...
        if not self.opt.lock_geo:
            # displacement
            if self.opt.geo_mlp:
                self.encoder_geo, in_dim_geo = get_encoder(
                    "hashgrid", interpolation="smoothstep"
                )
                self.geo_net = MLP(in_dim_geo, 1, 32, 2)
            else:
                if self.opt.pose_mlp is not None:
                    self.v_offsets = torch.zeros(N, 1, requires_grad=False).to(
                        self.device
                    )
                else:
        ...
        base_options = python.BaseOptions(
            model_asset_path="data/mediapipe/face_landmarker.task"
        )
    ...
    def get_init_body(self, cache_path="./data/init_body/data.npz"):
        if True:
            if self.num_remeshing == 1:
                ...
                faces_list = [
                    torch.as_tensor(f, device=self.device) for f in data["faces"]
                ]
                dense_lbs_weights = torch.as_tensor(
                    data["dense_lbs_weights"], device=self.device
                )
            ...
            dense_faces = np.concatenate(
                [dense_faces, self.smplx_faces[~self.remesh_mask]]
            )
            ...
            for _ in range(1, self.num_remeshing):
                dense_v_cano, dense_faces, dense_lbs_weights, unique = subdivide(
                    dense_v_cano, dense_faces, dense_lbs_weights
                )
            ...
            dense_v_posed = warp_points(
                dense_v, dense_lbs_weights, output.joints_transform[:, :55]
            )[0]
        ...
        if not self.opt.lock_geo:
            if self.opt.pose_mlp != "none" and self.opt.pose_mlp != "displacement":
                if not video:  # image case
                    ...
                    if self.opt.model_change:
                        ...
                        prediction = pose_mlp_output + self.init_body_pose_6d
                        ...
                        body_pose = (
                            self.body_prior.decode(prediction.unsqueeze(0))["pose_body"]
                            .contiguous()
                            .view(1, -1)
                        )
                else:
                    ...
                    if self.opt.pose_mlp == "pose":
                        pose_mlp_output = self.pose_mlp(
                            torch.tensor([frame_id], device=self.device)
                        )
                    elif self.opt.pose_mlp == "kickstart":
                        pose_mlp_output = self.pose_mlp(
                            torch.tensor([frame_id], device=self.device),
                            self.prev_pose[frame_id],
                        )
                    ...
                    if self.vpose:
                        ...
                        full_pose = rotation_6d_to_matrix(
                            prediction.view(-1, 55, 6)
                        ).view(-1, 55, 3, 3)
            ...
            # Non pose mlp case
                ...
                if self.opt.model_change:
                    if self.opt.use_full_pose:
                        ...
                        if not video:
                            ...
                            body_pose_6d = (
                                self.body_pose_6d_set[frame_id]
                                + self.init_body_pose_6d_set[frame_id]
                            )
                else:
                    if self.opt.use_full_pose:
                ...
                if self.opt.use_full_pose:
                    ...
                    global_orient = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[:1].view(-1, 6))
                    ).view(1, -1)
                    body_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[1:22].view(-1, 6))
                    ).view(1, -1)
                    jaw_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[22:23].view(-1, 6))
                    ).view(1, -1)
                    left_eye_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[23:24].view(-1, 6))
                    ).view(1, -1)
                    right_eye_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[24:25].view(-1, 6))
                    ).view(1, -1)
                    left_hand_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[25:40].view(-1, 6))
                    ).view(1, -1)
                    right_hand_pose = matrix_to_axis_angle(
                        rotation_6d_to_matrix(full_pose_6d[40:55].view(-1, 6))
                    ).view(1, -1)
                else:
                    if self.vpose:
                        body_pose = (
                            self.body_prior.decode(body_pose_6d.unsqueeze(0))[
                                "pose_body"
                            ]
                            .contiguous()
                            .view(1, -1)
                        ...
                        body_pose = matrix_to_axis_angle(
                            rotation_6d_to_matrix(body_pose_6d.view(-1, 21, 6))
                        ).view(1, -1)
            ...
            if not self.simplify:
                ...
                v_posed_dense = warp_points(
                    ...
                    output.joints_transform[:, :55],
                ).squeeze(0)
                # # if not is_train:
                # v_posed_dense = v_cano
                v_posed_dense, center, scale = normalize_vert(
                    v_posed_dense, return_cs=True
                )
                mesh = Mesh(
                ...
                v_cano, center, scale = normalize_vert(v_cano, return_cs=True)
                vertices, faces = fast_simplification.simplify(
                    v_cano.detach().cpu().numpy(), self.smplx_faces, 0.9
                )
                mesh_trimesh = trimesh.Trimesh(vertices, faces)
                vmapping, indices, uvs = xatlas.parametrize(
                    mesh_trimesh.vertices, mesh_trimesh.faces
                )
                ...
                mesh = Mesh(
                    vertices, torch.from_numpy(faces).cuda(), vt=uvs, ft=indices
                )
            mesh.auto_normal()
    ...
    def forward(
        ...
        if not self.opt.skip_bg:
            ...
            bg_color = (
                torch.sigmoid(self.bg_net(self.encoder_bg(dirs.view(-1, 3))))
                .view(batch, h, w, 3)
                .contiguous()
            )
        else:
        ...
        if self.opt.use_cubemap:
            ...
            for i in range(6):
                cube_map_texture[0, i] = torch.tensor(
                    np.array(
                        Image.open(
                            f"cubemaps/SanFrancisco4/{map_texture_locations[i]}"
                        ).resize((256, 256))
                    ).astype(np.float32)
                    / 255.0
                )
            ...
            pos_clip = torch.bmm(
                F.pad(pos, pad=(0, 1), mode="constant", value=1.0)
                .unsqueeze(0)
                .expand(1, -1, -1),
        ...
        if video:
            ...
            for i in range(frame_size):
                pr_mesh, smplx_landmarks, prediction, smplx_joints = self.get_mesh(
                    is_train=is_train, frame_id=i
                )
                ...
                rgb, normal, alpha = self.renderer(
                    pr_mesh,
                    mvp,
                    h,
                    w,
            ...
            pr_mesh, smplx_landmarks, prediction, joints = self.get_mesh(
                is_train=is_train
            )
            rgb, normal, alpha = self.renderer(
                pr_mesh,
            ...
            smplx_landmarks = torch.bmm(
                F.pad(
                    smplx_landmarks, pad=(0, 1), mode="constant", value=1.0
                ).unsqueeze(0),
                torch.transpose(mvp, 1, 2),
            ).float()  # [B, N, 4]
