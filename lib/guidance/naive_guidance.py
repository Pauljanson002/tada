...
class Naive(nn.Module):
    def __init__(self):
        ...
        print(f'[INFO] loading Naive guidance...')
        
        self.reference_videos = {i:torchvision.io.read_video(f"4d/reference_videos/{i}.mp4",pts_unit="sec")[0].permute(0,3,1,2).float().cuda()/255.0 for i in range(4)}
    ...
    def train_step(self, text_embeddings, pred_rgbt, guidance_scale=100, rgb_as_latents=False,**kwargs): # pred_rgbt: [F, 3, H, W]
        ...
        alpha_frames = kwargs.get("alpha_frames",None)
