...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run --multirun text="man" action="lifting the arm to top of head" wandb_mode="disabled" name=test training.debug=False guidance.name=zeroscope model.video=True model.num_frames=20 training.iters=400 training.accumulate=True training.regularize_coeff=0.0 \
