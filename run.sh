...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run --multirun text="man" action="running","lifting both arms above head","bending and touching his feet" wandb_mode="disabled" name=test training.debug=True hydra.verbose=True guidance.name=sd model.video=False model.num_frames=1 training.iters=1000 training.accumulate=True training.constraint_latent_weight=0.1,1.0 \
