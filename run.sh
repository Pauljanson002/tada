...
CUDA_VISIBLE_DEVICES=0 python -m apps.run text="man" action="running" wandb_mode="online" name=test training.debug=True guidance.name=naive model.video=True model.num_frames=5 training.iters=400 training.accumulate=True training.regularize_coeff=0.0 
