...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run --multirun text="man" action="vpose_prior" wandb_mode="online" name=test training.debug=True guidance.name=naive model.video=True model.num_frames=10 training.iters=1000 training.accumulate=True training.regularize_coeff=0.0 \
model.initialize_pose="diving" model.model_change=False data.h=256 data.w=256 model.use_cubemap=False
