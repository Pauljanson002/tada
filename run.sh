...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run --multirun text="man" action="running" wandb_mode="disabled" name=test training.debug=False hydra.verbose=True guidance.name=sd guidance.loss_type="standard" model.video=False model.num_frames=1 training.iters=5000 training.accumulate=False training.constraint_latent_weight=0.0 \
model.initialize_pose="zero" model.model_change=False data.h=256 data.w=256 model.use_cubemap=True fp16=True training.scheduler=False model.add_fake_movement=True training.set_global_time_step=True training.lr=0.05 training.guidance_scale=100 negative="" 
