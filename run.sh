...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run text="man" action="running" wandb_mode="disabled" name=test training.debug=True guidance.name=zeroscope model.video=True model.num_frames=4 training.iters=1000 training.accumulate=True training.regularize_coeff=0.0 \
