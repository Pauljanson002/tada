...
CUDA_VISIBLE_DEVICES=0 python -m  apps.run --multirun text="man" action="punching a forward jab" wandb_mode="disabled" name=test training.debug=False hydra.verbose=False guidance.name=modelscope model.video=True model.num_frames=20 training.iters=400 training.accumulate=True training.regularize_coeff=1.0 \
